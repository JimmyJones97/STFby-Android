...
import com.stczh.otherlibs.R;
import com.stczh.otherlibs.Waterfall.PLA_AbsListView;
import com.stczh.otherlibs.Waterfall.PLA_AdapterView;
...
public class PLA_ListView extends PLA_AbsListView {
    ...
    private void clearRecycledState(ArrayList<FixedViewInfo> infos) {
        if (infos != null) {
            ...
            for (int i = 0; i < count; i++) {
                PLA_AbsListView.LayoutParams p = (PLA_AbsListView.LayoutParams) infos.get(i).view.getLayoutParams();
                if (p != null) {
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (this.mItemCount > 0 && (widthMode == 0 || heightMode == 0)) {
            ...
            if (recycleOnMeasure() && this.mRecycler.shouldRecycleViewType(((PLA_AbsListView.LayoutParams) child.getLayoutParams()).viewType)) {
    ...
    private void measureScrapChild(View child, int position, int widthMeasureSpec) {
        ...
        if (p == null) {
            p = new PLA_AbsListView.LayoutParams(-1, -2, 0);
    ...
    public final int measureHeightOfChildren(int widthMeasureSpec, int startPosition, int endPosition, int maxHeight, int disallowPartialChildPosition) {
        ...
        while (i <= endPosition) {
            ...
            if (recyle && recycleBin.shouldRecycleViewType(((PLA_AbsListView.LayoutParams) child.getLayoutParams()).viewType)) {
    ...
    private void scrollListItemsBy(int amount) {
        ...
        if (amount < 0) {
            ...
            while (first.getBottom() < listTop) {
                if (recycleBin.shouldRecycleViewType(((PLA_AbsListView.LayoutParams) first.getLayoutParams()).viewType)) {
        ...
        while (last2.getTop() > listBottom) {
            if (recycleBin.shouldRecycleViewType(((PLA_AbsListView.LayoutParams) last2.getLayoutParams()).viewType)) {
