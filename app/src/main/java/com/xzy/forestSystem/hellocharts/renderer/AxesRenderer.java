...
import com.xzy.forestSystem.hellocharts.util.AxisAutoValues;
import com.xzy.forestSystem.hellocharts.util.ChartUtils;
import com.xzy.forestSystem.hellocharts.util.FloatUtils;
...
public class AxesRenderer {
    ...
    private boolean checkRawValue(Rect rect, float rawValue, boolean axisInside, int position, boolean isVertical) {
        ...
        if (isVertical) {
            return rawValue <= ((float) rect.bottom) - ((float) (this.labelTextAscentTab[3] + this.axisMargin)) && rawValue >= ((float) rect.top) + ((float) (this.labelTextAscentTab[0] + this.axisMargin));
        }
