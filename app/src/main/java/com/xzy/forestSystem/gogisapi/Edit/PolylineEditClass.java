...
public class PolylineEditClass extends ZoomInOutPan {
    ...
    boolean _AllowSnap;
    int _ClickCount;
    String _CurrentDrawType;
    long _FirstClickTime;
    boolean _IsEnabelDraw;
    boolean _IsHandFreeMode;
    List<Double> _LengthList;
    Paint _LineEndVPaint;
    Paint _LineVertexPaint01;
    Paint _LineVertexPaint02;
    Polyline _Polyline;
    long _SecondClickTime;
    double _SnapDistance;
    Paint _TextBgPaint;
    Paint _TextPaint;
    Paint _linePaint;
    List<Coordinate> m_MeasurePointList;
    private int m_MeasurePtnCount;
    List<Coordinate> m_ResMeasurePointList;

    public PolylineEditClass(MapView mapView) {
        super(mapView);
        this._ClickCount = 0;
        ...
        this._Polyline.SetAllCoordinateList(this.m_MeasurePointList);
    ...
    public void AddPoint(Coordinate coordinate) {
        if (this.m_MeasurePointList.size() <= 0 || !this.m_MeasurePointList.get(this.m_MeasurePointList.size() - 1).Equal(coordinate)) {
            ...
            double tmpTotalLen = this._Polyline.getLength(true);
            if (this.m_MeasurePointList.size() == 1) {
                this._LengthList.add(Double.valueOf(0.0d));
            } else {
                this._LengthList.add(Double.valueOf(tmpTotalLen));
            }
    ...
    public boolean RedoLastPoint(String[] outMsg) {
        ...
        if (this.m_MeasurePointList.size() < this.m_ResMeasurePointList.size()) {
            ...
            this._LengthList.add(Double.valueOf(this._Polyline.getLength(true)));
            this._mapView.invalidate();
            this.m_MeasurePtnCount++;
            return true;
        }
    ...
    public boolean ReversCoords() {
        ...
        Collections.reverse(this._LengthList);
            ...
            if (this.m_MeasurePointList.size() != 0) {
                if (this.m_MeasurePointList.size() == 1) {
                    ...
                    if (tmpPoint.x >= 0 && tmpPoint.x <= PubVar.ScreenWidth && tmpPoint.y >= 0 && tmpPoint.y <= PubVar.ScreenHeight) {
                        paramCanvas.drawCircle((float) tmpPoint.x, (float) tmpPoint.y, (float) (EditingLineNodeSize + 5), this._LineVertexPaint01);
                        paramCanvas.drawCircle((float) tmpPoint.x, (float) tmpPoint.y, (float) EditingLineNodeSize, this._LineVertexPaint02);
                        paramCanvas.drawCircle((float) tmpPoint.x, (float) tmpPoint.y, (float) EditingLineNodeCircleSize, this._LineEndVPaint);
                        return;
                    }
                    return;
                }
                ...
                if (tmpLinePtns != null && tmpLinePtns.length > 0) {
                    ...
                    if (tmpEndPtn.x >= 0 && tmpEndPtn.x <= PubVar.ScreenWidth && tmpEndPtn.y >= 0 && tmpEndPtn.y <= PubVar.ScreenHeight) {
                        paramCanvas.drawCircle((float) tmpEndPtn.x, (float) tmpEndPtn.y, (float) EditingLineNodeCircleSize, this._LineEndVPaint);
                    }
                    int length = tmpLinePtns.length;
                    for (int i2 = 0; i2 < length; i2++) {
                        Point tmpPoint2 = tmpLinePtns[i2];
                ...
                double tempLength = this._Polyline.getLength(false);
                if (tempLength > 0.0d) {
                    String tempMgs = "【" + this._CurrentDrawType + "】线:" + Common.SimplifyLength(tempLength, true);
